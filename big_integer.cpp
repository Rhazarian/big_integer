...
struct helper_functions {
  ...
  static void add_uint(big_integer& x, uint32_t val) // x  in sign-magnitude representation
  {
      for (auto it = x.data.begin(); val != 0 && it != x.data.end(); ++it) {
          auto overflow = val > UINT32_MAX - *it;
          *it += val;
          val = overflow;
      }
      if (val)
  ...
  static int8_t cmp_abs(big_integer const& lhs, big_integer const& rhs) // lhs, rhs - in sign-magnitude representation
  {
      int8_t val = 0;
...
big_integer operator+(big_integer const& lhs, big_integer const& rhs)
{
    ...
    for (big_integer::container_t::size_type i = 0; i < min_len; ++i) {
        bool overflow = lhs.data[i] > UINT32_MAX - rhs.data[i] || (carry && lhs.data[i] == UINT32_MAX - rhs.data[i]);
    ...
    if (same_sign) {
        if (lhs_sign)
            res.data.emplace_back(UINT32_MAX);
        else
    ...
    for (big_integer::container_t::size_type i = 0; i < rhs.data.size(); ++i) {
        ...
        for (big_integer::container_t::size_type j = i; j < i + tmp.data.size(); ++j) {
            bool overflow = res.data[j] > UINT32_MAX - tmp.data[j - i]
                    || (carry && res.data[j] == UINT32_MAX - tmp.data[j - i]);
...
big_integer operator<<(big_integer const& lhs, int val) // lhs - is negative --> UB
{
    ...
    val %= 32;
    big_integer res(lhs.data.size() + skip, true);
    uint32_t carry = 0;
